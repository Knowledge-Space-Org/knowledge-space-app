...
const FacetLinks = ({ slug, classes, aggs, facets, selected, handleFacetToggle }) => (
  <Grid
    container
...
class FacetLink extends Component {
  ...
  render() {
    const { selected, classes, name, values, title, slug } = this.props;
    return (
      <Grid item className={classes.gridItem}>
        <Paper color="#fff">
...
export default withStyles(styles)(FacetLinks);
